<script>
	...
	const games = [
		...
		},
		{
			...
			date: 'TBA',
			platform: 'PC',
			img: '/games/quack_quack_quack.png',
			genres: 'Typing | Puzzle | Adventure | Storytelling',
			description: `Quack quack quaaack, quack quack quack-quack quaaaack. Quack... quack quack, quack! Quack quack quack quaaack, quack-quack. Quaaaaaack, quack quack? Quack! Quack quack quack. 
				<br/>
				<br/>
				Quack' Quack, Quack is a 2D typing puzzle game about relearning how to communicate — but not as a human — in order to save your duck girlfriend from the commercial industry.
			`
		}
	];
</script>
				...
				<div style="display: flex; flex-direction: row-reverse; gap: 1rem; margin-bottom: 10px;">
					...
					>
						...
						<button
							...
							class="base-hover"
							style="background-color: transparent; border: none; opacity: 0.5; color:white; z-index: 0; text-align: right;"
							>Know more about {member.pronoum}</button
						>
					</div>
				</div>
			{/each}
			...
			{#each games as game}
				<div class="games-div-game-info">
					...
					>
						...
						<div
							style={game === games[games.length - 1] ? 'width: 0%;' : 'width: 100%; '}
							class="divider"
						/>
					</div>
				</div>
	...
	<div id="contact-div" class="content-div jost-light-300">
		<div style="display: flex; flex-direction: column; justify-content: center; align-items:left;">
			...
			<div style="display: flex; justify-content: center; align-items:center; gap: 2rem;">
				...
				<button
					title="Redirect to Wise Shepherd Games's LinkedIn Page."
					class="contact-buttons base-hover"
					on:click={() => {
						helpers.DomHelper.redirect('https://www.linkedin.com/company/wise-shepherd-games');
					}}
				>
					<img style="width: 75%; height: auto;" src="linkedin_logo.png" alt="LinkedIn Logo" />
				</button>
			</div>
			<br />
